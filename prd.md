...
- **Acceptance criteria:**
  ...
  - [ ] [GSC 2026-09-07 — BELUM TERPENUHI] Google Search Console Coverage report menunjukkan 2 masalah indexing aktif dengan validasi Gagal: "Halaman dengan pengalihan" (3 URL — varian `http://`/`/index.html` dari homepage) dan "Di-crawl - saat ini tidak diindeks" (16 URL — varian `/game.html?id=`/`/game?id=` yang seharusnya sudah 301 ke `/play/:id/:slug` per kriteria di atas, tapi data GSC mengindikasikan sebaliknya). Kriteria "URL bersih tanpa ekstensi `.html`" di atas belum sepenuhnya tercapai di mata Googlebot — lihat changelog.md Task #020/#021.
